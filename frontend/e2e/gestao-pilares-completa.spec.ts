...
test.describe.skip('LEGACY: Modal Gerenciar Pilares - Funcionalidades Completas @pilares @regression @high @legacy', () => {
  ...
  test('ADMINISTRADOR deve remover pilar da empresa e validar exclusão', async ({ page }) => {
    ...
    const drawerTitle = page.locator('.offcanvas-title:has-text("Gerenciar Pilares da Empresa"), .offcanvas-header:has-text("Gerenciar Pilares da Empresa"), [data-testid="drawer-title"]:has-text("Gerenciar Pilares da Empresa"), [data-testid="offcanvas-title"]:has-text("Gerenciar Pilares da Empresa")');
  ...
  test('GESTOR deve ver apenas pilares da própria empresa no modal', async ({ page }) => {
    ...
    const ngSelect = page.locator('.offcanvas-body ng-select, [data-testid="drawer-body"] ng-select, [data-testid="offcanvas-body"] ng-select').first();
    await ngSelect.click();
    await page.waitForTimeout(500);

    const searchInput = page.locator('.offcanvas-body ng-select input[type="text"], [data-testid="drawer-body"] ng-select input[type="text"], [data-testid="offcanvas-body"] ng-select input[type="text"]');
