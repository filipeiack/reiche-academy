...
test.describe('Diagnóstico de Notas', () => {
  ...
  test('deve preencher e salvar notas de rotinas', async ({ page }) => {
    // Aguardar carregamento dos pilares
    await page.waitForSelector('[data-testid="pilar-accordion"]');
    ...
    // Procurar inputs de nota dentro do pilar
    ...
    if (await notaInputs.first().isVisible()) {
      ...
      // Preencher criticidade (se houver select)
      const criticidadeSelect = firstPilar.locator('select');
      if (await criticidadeSelect.first().isVisible()) {
        await criticidadeSelect.first().selectOption('ALTO');
      }
      ...
      if (await savingBar.isVisible()) {
        const savingIndicator = page.getByTestId('saving-indicator');
        ...
        const lastSaveInfo = page.getByTestId('last-save-info');
        if (await lastSaveInfo.isVisible()) {
          await expect(lastSaveInfo).toBeVisible();
        }
      }
    } else {
      console.log('Inputs de nota não encontrados no primeiro pilar');
    }
  });

  test('deve calcular progresso automaticamente', async ({ page }) => {
    // Aguardar carregamento dos pilares
    await page.waitForSelector('[data-testid="pilar-accordion"]');
    
    // Verificar se há badges de progresso
    const progressBadges = page.locator('.badge:has-text("Tx respostas")');
    
    if (await progressBadges.first().isVisible()) {
      // Verificar se há valor de progresso
      const firstBadge = progressBadges.first();
      const badgeText = await firstBadge.textContent();
      
      // Verificar se o texto contém percentual
      expect(badgeText).toMatch(/\d+%/);
      
      // Preencher algumas notas para ver se o progresso atualiza
      const firstPilar = page.locator('[data-testid="pilar-accordion"]').first();
      const isCollapsed = await firstPilar.locator('.collapsed').count() > 0;
      if (isCollapsed) {
        await firstPilar.locator('button').click();
      ...
      const notaInputs = firstPilar.locator('input[type="number"]');
      if (await notaInputs.first().isVisible()) {
        await notaInputs.first().fill('7');
        await page.waitForTimeout(2000);
        
        // Verificar se o progresso foi atualizado
        const newBadgeText = await firstBadge.textContent();
        expect(newBadgeText).toMatch(/\d+%/);
      }
    } else {
      console.log('Badges de progresso não encontrados');
    }
  });
