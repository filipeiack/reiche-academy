...
update_build_version_in_env() {
    ...
    if [ ! -f "$ENV_FILE" ]; then
        echo -e "${YELLOW}⚠️  Arquivo $ENV_FILE não encontrado, criando...${NC}" >&2
    ...
    echo -e "${GREEN}✅ .env atualizado com BUILD_VERSION=${new_version}${NC}" >&2
}
...
save_version() {
    ...
    echo -e "${GREEN}✅ Versão $version salva em $version_file${NC}" >&2
}
...
bump_version() {
    ...
    case $bump_type in
        ...
        patch)
            ...
            ;;
        *)
            echo -e "${RED}❌ Tipo inválido: $bump_type (use: major, minor, patch)${NC}" >&2
            exit 1
            ;;
    esac
...
EOF
    ...
    ln -sf "$(basename "$metadata_file")" "${DEPLOY_METADATA_DIR}/current-${env}.json" 2>/dev/null || true
    
    echo -e "${GREEN}✅ Metadata criada: $metadata_file${NC}" >&2
}
...
case $ACTION in
    ...
    *)
        echo -e "${RED}❌ Ação inválida: $ACTION${NC}" >&2
        echo "" >&2
        echo "Uso:" >&2
        echo "  bash version-manager.sh get [staging|prod]" >&2
        echo "  bash version-manager.sh bump [patch|minor|major] [staging|prod]" >&2
        echo "  bash version-manager.sh set <version> [staging|prod]" >&2
        echo "  bash version-manager.sh current [staging|prod]" >&2
        echo "  bash version-manager.sh history [staging|prod]" >&2
        echo "" >&2
        echo "Exemplos:" >&2
        echo "  bash version-manager.sh get staging          # Obter versão atual" >&2
        echo "  bash version-manager.sh bump patch prod      # Incrementar patch (1.0.0 → 1.0.1)" >&2
        echo "  bash version-manager.sh bump minor staging   # Incrementar minor (1.0.5 → 1.1.0)" >&2
        echo "  bash version-manager.sh set 2.0.0 prod       # Definir versão específica" >&2
        echo "  bash version-manager.sh current staging      # Mostrar info da versão atual" >&2
        echo "  bash version-manager.sh history              # Listar todos deploys" >&2
        exit 1
        ;;
esac
